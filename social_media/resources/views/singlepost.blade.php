<div id="post{{ $post -> PostID }}" class="post border-radius-5">
		...
		@endif
		<br>
		<div id="carouselExampleControls{{ $post -> PostID }}" class="carousel slide" data-ride="carousel">
		  <div class="carousel-inner">
			@if (isset($post->Images[0]))
			  <div id="carouselitem{{ $post -> Images[0] -> ImageID}}" class="carousel-item active">
			@else
			...
			@endif
			  @if (isset($post->Images[0]))
				...
				@if (pathinfo('/content/'.$post->Images[0]->ImageFile, PATHINFO_EXTENSION) == 'png' ||
					...
					<img src="{{ asset('/content/'.$post->Images[0]->ImageFile) }}" class="d-block mx-auto" alt="No image found">
				@elseif (pathinfo('/content/'.$post->Images[0]->ImageFile, PATHINFO_EXTENSION) == 'mp4' ||
					...
					<video class="d-block mx-auto" alt="No video found" controls>
				...
				@elseif (pathinfo('/content/'.$post->Images[0]->ImageFile, PATHINFO_EXTENSION) == 'mp3' ||
					...
					<audio src="{{ asset('/content/'.$post->Images[0]->ImageFile) }}" class="d-block mx-auto" alt="No video found"></audio>
			    @endif
			  @else
				<img src="{{ asset('content/noimage.png') }}" class="d-block mx-auto" alt="No image found">
			  @endif
			  @if ($post -> PersonID == session() -> get('PersonID'))
			...
			@for ($ImageFile = 1 ; $ImageFile < count($post -> Images) ; $ImageFile++)
				<div id="carouselitem{{ $post -> Images[$ImageFile] -> ImageID}}" class="carousel-item">
				  ...
				  @if (pathinfo('/content/'.$post->Images[$ImageFile]->ImageFile, PATHINFO_EXTENSION) == 'png' ||
					...
					<img src="{{ asset('/content/'.$post->Images[$ImageFile]->ImageFile) ?? asset('/content/noimage.png') }}" class="d-block mx-auto" alt="No image found">
				  @elseif (pathinfo('/content/'.$post->Images[$ImageFile]->ImageFile, PATHINFO_EXTENSION) == 'mp4' ||
					...
					<video width="320" height="240" class="d-block mx-auto" controls>
